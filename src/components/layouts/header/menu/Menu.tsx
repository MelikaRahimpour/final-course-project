...
import {browsCategoriesMock} from "@/mock/browsCategoriesMock";
                    ...
                    <div id="all_cat_inner_box" className="flex flex-wrap justify-between gap-y-[15px]">
                        {
                            browsCategoriesMock.map((item, index) => {
                                return <IconBox key={index} link={item.link} linkClassName={"gap-3.5 rounded-[5px] lg:border-[1px] lg:border-gray-300 py-2.5 basis-[calc(50%-8px)] justify-start pl-4 lg:hover:border-green-300"} icon={item.icon} size={30} path={item.iconPath} title={item.title} titleClassName={"text-heading-sm text-blue-300"}/>
                            })
                        }
                        <IconBox linkClassName={"gap-4 justify-center w-full mt-[17px]"} icon={"icon-add"}  title={"More Categories"} titleClassName={"text-heading-sm text-blue-300"}/>
