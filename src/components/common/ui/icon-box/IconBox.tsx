...
export function IconBox({icon, size = 22 , link, title, hideTitleOnMobile, badge = 0, titleClassName, path = 0, linkClassName}: Props ) {

    let span = [];
    for (let i : number = 1; i <= path; i++) {
        span.push(<span className={`path${i}`}></span> )
    }
    if (link){
        return (
            <Link href={link ?? '#'} className={`flex items-center cursor-pointer ml-1 ${linkClassName}`}>
                {
                    badge ?
